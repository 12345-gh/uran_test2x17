...
export default function (app) {
	app.service('MaskService', MaskService);

	function MaskService($q, ArrayService) {
		"ngInject";
		...
		this.create = () => {
			...
			let getOptionalsRelativeMaskWithoutOptionals = (optionals) => {
				let indexes = [];
			...
			let isOptional = (currentPos) => {
				return ArrayService.inArray(currentPos, optionalIndexes);
			};

			let isDivisor = (currentPos) => {
				return ArrayService.inArray(currentPos, divisors);
			};
			...
			let removeDivisors = (value) => {
				...
				try {
					if (divisors.length > 0 && value) {
						...
						elments = ArrayService.uniqueArray(elments);
			...
			let insertDivisors = (array, combination) => {
				...
				let divs = divisors.filter(it => {
					...
					return !ArrayService.inArray(it, combination) && !ArrayService.inArray(it, optionalDivisorsKeys);
				});
			...
			let tryDivisorConfiguration = (value) => {
				...
				if (optionalIndexes.length > 0) {
					...
					if (optionalDivisorsCombinations.length === 0) {
						ArrayService.lazyProduct(lazyArguments, () => {
